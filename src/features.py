...
MIN_RETAILERS_FOR_CV = 2  # can't measure cross-retailer variability with fewer than 2 retailers
...
def _fill_gaps(g: pd.DataFrame) -> pd.DataFrame:
    ...
    zero_cols = [
        "units", "sales", "promo_units", "promo_amt", "acv", "promo_acv",
        "tpr_only", "feat_display", "feat_only", "display_only",  # promo-mechanic ACV, same treatment as promo_acv
    ]
...
def build_retailer_weekly_panel(df: pd.DataFrame) -> pd.DataFrame:
    """Weekly data at the PPG × retailer level, with missing weeks filled in. This level of detail is needed for retailer-level analyses and features that 
    measure variation across retailers"""
    pr_weekly = (
        ...
        .agg(
            ...
            # promo mechanic mix (see _promo_features) - kept at retailer grain
            tpr_only=("acv_tpr_only", "mean"),
            feat_display=("feature_and_display_acv_pct", "mean"),
            feat_only=("feature_without_display_acv_pct", "mean"),
            display_only=("display_without_feature_acv_pct", "mean"),
        )
        .reset_index()
...
def _build_weekly_panel(df: pd.DataFrame) -> pd.DataFrame:
    ...
    weekly = (
        ...
        .agg(
            ...
            tpr_only=("tpr_only", "mean"),
            feat_display=("feat_display", "mean"),
            feat_only=("feat_only", "mean"),
            display_only=("display_only", "mean"),
        )
        .reset_index()
...
def _price_regression(g: pd.DataFrame) -> pd.Series:
    """Elasticity, controlling for promo weeks (price and promo both move together). Could be improved with a more sophisticated model (e.g. lagged price, ARIMA, etc.), 
    but this is a simple and interpretable baseline. Returns NaN if there are too few observations or no price variation"""
...
def _promo_features(g: pd.DataFrame) -> pd.Series:
    """Promotion-related features for a PPG"""
    ...
    visibility_acv = g["feat_display"] + g["feat_only"] + g["display_only"]
    promo_visibility_share = (
        (visibility_acv / g["promo_acv"].replace(0, np.nan))
        .replace([np.inf, -np.inf], np.nan)
        .mean()
    )
    return pd.Series({
        ...
        "promo_visibility_share": promo_visibility_share,
    })


def _windowed_growth(g: pd.DataFrame, window: int) -> float:
    """Used for both momentum (12) and MAT growth (52)."""
    if len(g) < 2 * window:
        return np.nan
...
def _cv(s: pd.Series) -> float:
    """Compute the coefficient of variation (CV) for a series, returning `NaN` when there are too few observations or the mean is zero"""
    s = s.dropna()
    if len(s) < MIN_RETAILERS_FOR_CV or s.mean() == 0:
        return np.nan
    return float(s.std() / abs(s.mean()))


def _retailer_variability(pr_weekly: pd.DataFrame) -> pd.DataFrame:
    """Measure how much a PPG's promo effectiveness and distribution vary across retailers. The metrics are calculated at the retailer level, 
    then summarized as the coefficient of variation across retailers. Single-retailer PPGs return 0 because there is no retailer-to-retailer variation"""
    per_retailer = (
        pr_weekly.groupby(["rgm_ppg", "retailer_nm"])
        .apply(_promo_features, include_groups=False)
        .reset_index()
    )
    acv_per_retailer = (
        pr_weekly.groupby(["rgm_ppg", "retailer_nm"])["acv"].mean().rename("acv").reset_index()
    )
    per_retailer = per_retailer.merge(acv_per_retailer, on=["rgm_ppg", "retailer_nm"])

    out = per_retailer.groupby("rgm_ppg").agg(
        promo_lift_retailer_cv=("promo_lift", _cv),
        acv_retailer_cv=("acv", _cv),
    )
    return out


def _residuals_by_retailer(pr_weekly: pd.DataFrame) -> pd.DataFrame:
    """Calculate log-sales residuals relative to each PPG's non-promo baseline, separately for each retailer. This keeps retailer-specific promo patterns independent"""
    out = pr_weekly.copy()
...
def _peer_correlation_matrix(pr_weekly_resid: pd.DataFrame, min_overlap: int = PEER_MIN_OVERLAP_WEEKS):
    """Calculate the correlation between PPG pairs by combining retailer-level correlations with Fisher's z transformation. Returns both the combined 
    correlation and the effective sample size, and is used for both peer and substitute relationships"""
...
def _pairwise_group_mask(index: pd.Index, same: list[pd.Series] = (), different: list[pd.Series] = ()) -> np.ndarray:
    """Build a boolean matrix identifying valid PPG pairs based on matching and differing attributes. Used to define both peer (same brand) and substitute (same subcategory, 
    different brand) relationships."""
    n = len(index)
    mask = np.ones((n, n), dtype=bool)
    for s in same:
        vals = np.asarray(s.reindex(index), dtype=object)
...
def _significant_pairs_mask(corr: pd.DataFrame, effective_n: pd.DataFrame,
                             n_pairs: int, alpha: float = 0.05) -> np.ndarray:
    """Apply a Bonferroni correction based on the number of tested PPG pairs, controlling for multiple comparisons within the relevant relationship set"""
    if n_pairs == 0:
        return np.zeros(corr.shape, dtype=bool)
    ...
    return p < alpha_corrected


def _grouped_correlation_feature(corr: pd.DataFrame, n_eff: pd.DataFrame, group_mask: np.ndarray,
                                  alpha: float = 0.05, name: str = "correlation") -> pd.Series:
    """Average significant correlation for each PPG against the peers
    defined by group_mask's row for that PPG"""
    n_pairs = int(group_mask.sum()) // 2
    sig = _significant_pairs_mask(corr, n_eff, n_pairs, alpha=alpha)
    keep = sig & group_mask
    with np.errstate(invalid="ignore", divide="ignore"):
        z = np.arctanh(corr.values.clip(-0.999, 0.999))
    z = np.where(keep, z, np.nan)
    out = pd.Series(index=corr.index, dtype=float, name=name)
    for i in range(len(corr.index)):
        row = z[i, :]
        valid = row[~np.isnan(row)]
        out.iloc[i] = np.tanh(valid.mean()) if len(valid) else np.nan
    return out
...
def _peer_correlation(df: pd.DataFrame, brand: pd.Series) -> pd.Series:
    ...
    a halo/product-line signal. Brand, not subcategory"""
    pr_weekly = build_retailer_weekly_panel(df)
    resid = _residuals_by_retailer(pr_weekly)
    corr, n_eff = _peer_correlation_matrix(resid)
    group_mask = _pairwise_group_mask(corr.index, same=[brand.reindex(corr.index)])
    return _grouped_correlation_feature(corr, n_eff, group_mask, name="peer_correlation")


def _substitute_correlation(df: pd.DataFrame, subcategory: pd.Series, brand: pd.Series) -> pd.Series:
    """Each PPG's average significant correlation with same-SUBCATEGORY,
    different-BRAND peers -- the classic substitute/cannibalization
    signal named in the brief"""
    pr_weekly = build_retailer_weekly_panel(df)
    resid = _residuals_by_retailer(pr_weekly)
    corr, n_eff = _peer_correlation_matrix(resid)
    group_mask = _pairwise_group_mask(
        corr.index,
        same=[subcategory.reindex(corr.index)],
        different=[brand.reindex(corr.index)],
    )
    return _grouped_correlation_feature(corr, n_eff, group_mask, name="substitute_correlation")
...
def _compute_features(weekly: pd.DataFrame, df: pd.DataFrame) -> pd.DataFrame:
    ...
    level["is_new"] = ((global_max - level["first_seen"]).dt.days < LIFECYCLE_GAP_WEEKS * 2 * 7).astype(int)
    level["is_delisted"] = ((global_max - level["last_seen"]).dt.days > LIFECYCLE_GAP_WEEKS * 7).astype(int)

    recent_avg_sales = (
        grouped["sales"].apply(lambda s: s.tail(MOMENTUM_WINDOW_WEEKS).mean()).rename("recent_avg_sales")
    )
    ...
    pr_weekly = build_retailer_weekly_panel(df)
    retailer_var = _retailer_variability(pr_weekly)

    feats = (
        level.drop(columns=["first_seen", "last_seen", "sales_std"])
        .join(recent_avg_sales)
        .join(growth)
        .join(acv_trend)
        .join(momentum)
        .join(mat_growth)
    ...
    fillable = [
        "price_elasticity", "promo_effect_ctrl", "promo_lift", "momentum",
        "seasonality_index", "promo_visibility_share",
    ]
    for col in fillable:
        feats[col + "_missing"] = feats[col].isna().astype(int)
        feats[col] = feats[col].fillna(feats[col].median()).fillna(0)

    feats["mat_growth_missing"] = feats["mat_growth"].isna().astype(int)
    feats["mat_growth"] = feats["mat_growth"].fillna(feats["momentum"])
    feats["mat_growth"] = feats["mat_growth"].fillna(1.0)  # true no-signal cases only

    # retailer variability: 0-fill is the correct value here
    for col in ["promo_lift_retailer_cv", "acv_retailer_cv"]:
        feats[col + "_missing"] = feats[col].isna().astype(int)
    ...
    meta = (
        ...
        .set_index("rgm_ppg")[[
            ...
            "product_unit_size", "product_pack_size", "product_pack_count", "product_is_multipack",
        ]]
        .rename(columns={"attribute_9": "price_tier", "attribute_1": "form", "attribute_7": "benefit_type"})
    )
    meta["product_is_multipack"] = (meta["product_is_multipack"] == "YES").astype(int)
    feats = feats.join(meta)

    # within-PPG interaction terms
    ...
    # peer_correlation: average residual-sales correlation with same-BRAND peers (halo signal)
    ...
    # substitute_correlation: average residual-sales correlation with same-SUBCATEGORY
    sub_corr = _substitute_correlation(df, feats["subcategory_nm"], feats["brand_nm"])
    feats["substitute_correlation_missing"] = sub_corr.reindex(feats.index).isna().astype(int)
    feats["substitute_correlation"] = sub_corr.reindex(feats.index).fillna(sub_corr.median()).fillna(0)
